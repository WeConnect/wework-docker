node["deploy"].each do |application, deploy|
  if deploy[:application_type] != 'other' || deploy["environment_variables"]["APP_TYPE"] != 'docker'
    Chef::Log.info("Skipping deploy::docker application #{application} as it is not deployed to this layer")
    next
  end
  ...
  deploy["containers"].each do |c|
    c.each do |app_name, app_config|
      ...
      Chef::Log.info("Deploying '#{application}/#{app_name}', from '#{image}'")
      ...
      containers.to_i.times do |i|
        ...
        execute "migrate #{app_name}#{i} container" do
          ...
          command "docker run --rm #{e.env_string(environment, i)} #{e.links} #{e.volumes} #{e.volumes_from} #{image} #{app_config["migration"]}"
        ...
        execute "launch #{app_name}#{i} container" do
          ...
          command "docker run -d -h #{e.hostname i} --name #{app_name}#{i} #{e.ports} #{e.env_string(environment, i)} #{e.links} #{e.volumes} #{e.volumes_from} #{e.entrypoint} #{image} #{e.cmd i}"
        ...
        cron "#{app_name}#{i} cron" do
          ...
          command "docker run --rm --name #{app_name}#{i} #{e.env_string(environment, i)} #{e.links} #{e.volumes} #{e.volumes_from} #{e.entrypoint} #{image} #{app_config["command"]}"
          only_if { e.cron? && i==0 }
        end
